...
begin
  ...
  if form1.cb33.checked=true then
      begin
        if winkel1d=0 then winkel1:=winkel1+a;
        if winkel1d=1 then winkel1:=winkel1-a;
      end;
...
procedure Begrenzung;
begin
  if xneu>255 then begin xneu:=255;   end;
  if yneu>255 then begin yneu:=255;   end;
...
procedure Begrenzung3;
begin
  if xneu>255 then begin xneu:=255;Zwangsblank:=true  end;
  if yneu>255 then begin yneu:=255;Zwangsblank:=true  end;
  if xneu < 0 then begin xneu:=0;Zwangsblank:=true   end;
  if yneu < 0 then begin yneu:=0;Zwangsblank:=true    end;
end;
...
if (status=1) then
 begin
   portout(lptport,port);  ASMNOP;
   portout(lptport,5);     ASMNOP;
   portout(lptport,13);    ASMNOP;

   portout(lptport,psw);   ASMNOP;
   portout(lptport,12);    ASMNOP;
   portout(lptport,13);    ASMNOP;
 end;
end;
...
if form1.cb14.checked=true then
   if port=1 then
    begin
       // Mit Blanking
       portout(lptport+2,5);ASMNOP;  //Alles sperren
       portout(lptport,xneu);ASMNOP; //Daten X
       portout(lptport+2,7);ASMNOP;  //Oben Daten rein
       portout(lptport+2,5);ASMNOP;  //Alles Sperren
       portout(lptport,yneu);ASMNOP; //Daten Y
       portout(lptport+2,1);ASMNOP;  //Alles raus
    end else
    begin
      // ohne Blanking
      portout(lptport+2,4);ASMNOP;  //Alles sperren
      portout(lptport,xneu);ASMNOP; //Daten X
      portout(lptport+2,6);ASMNOP;  //Oben Daten rein
      portout(lptport+2,4);ASMNOP;  //Alles Sperren
      portout(lptport,yneu);ASMNOP; //Daten Y
      portout(lptport+2,0);ASMNOP; //Alles raus
    end
    else portout(lptport+2,1);ASMNOP;// Ende enable Output}
    // Pause Neu
...
begin
 if form1.cb38.checked=false then
   begin
     if form1.cb37.checked=true then portout(lptport+2,0) else portout(lptport+2,1);
   end else
   begin
      portout(lptport,1);ASMNOP;
      portout(lptport+2,5);ASMNOP;
      portout(lptport+2,13);ASMNOP;
      portout(lptport,0);ASMNOP;
      portout(lptport+2,8);ASMNOP;
      portout(lptport+2,13);ASMNOP;
   end;
end;
...
if form1.cb38.checked=false then
 begin
  portout(lptport+2,1);sleep(10); //Alles raus
  portout(lptport,128);sleep(10);   //Daten X
  portout(lptport+2,7);sleep(10);  //Oben Daten rein
  portout(lptport+2,5) ;sleep(10);   //Alles Sperren
  portout(lptport,128) ;sleep(10);   //Daten Y
  portout(lptport+2,1) ;sleep(10);   //Alles raus  }
  Blank_invert;
 end else
 begin
  portout(lptport+2,13);sleep(10);   //Alles raus
  portout(lptport,128);sleep(10);  //Daten X
  portout(lptport+2,15);sleep(10);   //Oben Daten rein
  portout(lptport+2,13);sleep(10);   //Alles Sperren
  portout(lptport,128) ;sleep(10);  //Daten Y
  portout(lptport+2,9) ;sleep(10);   //Alles raus  }
  Blank_invert;
 end;
end;
...
var f:integer;
begin
    if Form1.cb37.checked=false then f:=1 else F:=254;
    //Strahlschalter updaten
    portout(lptport+2,13);ASMNOP;
    portout(lptport,f);  ASMNOP;
    portout(lptport+2,5); ASMNOP;
    portout(lptport+2,13); ASMNOP;
    portout(lptport,psw ); ASMNOP;
    portout(lptport+2,12); ASMNOP;
    portout(lptport+2,13); ASMNOP;

    // D/A Ausgabe
    portout(lptport+2,13);  ASMNOP;  //Alles sperren
    portout(lptport,128);  ASMNOP;  //Daten X
    portout(lptport+2,15);ASMNOP; //Oben Daten rein
    portout(lptport+2,13);  ASMNOP; //Alles Sperren
    portout(lptport,128);    ASMNOP; //Daten Y
    portout(lptport+2,9);    ASMNOP;  //Alles raus
...
begin
  application.ProcessMessages;
  portout(lptport+2,4+laseran);ASMNOP;
  portout(lptport,xneu);      ASMNOP;
  portout(lptport+2,6+laseran);ASMNOP;
  portout(lptport+2,4+laseran);ASMNOP;
  portout(lptport,yneu);      ASMNOP;
  portout(lptport+2,0+laseran);ASMNOP;
end;
...
var a,b:integer;
begin
  for a:=1 to (form1.sb7.position*1000) do b:=b+1;
end;
